...
for (const name of ['index.html', 'privacy.html']) {
  ...
  for (const pattern of [/<!doctype html>/i, /<html lang="ko">/, /name="viewport"/, /name="description"/, /<title>.+<\/title>/, /id="main"/, /mailto:cryforthefutur@gmail.com/, /tts/, /YouTube/]) { assert.match(html, pattern, name); checks++; }
